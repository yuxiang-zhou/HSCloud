...
class index_EweiShopV2Page extends PluginMobilePage
{
	...
	public function lottery_info()
	{
		...
		if ($id) {
			...
			$log = pdo_fetchall('SELECT l.*,m.`nickname`,m.`avatar` FROM ' . tablename('ewei_shop_lottery_log') . ' AS l LEFT JOIN ' . tablename('ewei_shop_member') . ' AS m ON m.openid=l.join_user WHERE l.uniacid=:uniacid AND l.lottery_id=:lottery_id AND l.is_reward=1 ORDER BY addtime desc LIMIT 5', array(':uniacid' => $_W['uniacid'], ':lottery_id' => $id));
