...
<?php (!empty($this) && $this instanceof WeModuleSite || 0) ? (include $this->template('extension/pc-tabs', TEMPLATE_INCLUDEPATH)) : (include template('extension/pc-tabs', TEMPLATE_INCLUDEPATH));?>
<div class="clearfix template">
	<?php  if($do == 'installed') { ?>
		<h5 class="page-header">已安装的PC站模板</h5>
		<nav role="navigation" class="navbar navbar-default">
			<div class="container-fluid">
				<div class="navbar-header">
					<a href="javascript:;" class="navbar-brand">PC站模板类型</a>
				</div>
				<ul class="nav navbar-nav nav-btns">
					<li <?php  if($_GPC['type'] == 'all' || empty($_GPC['type'])) { ?> class="active" <?php  } ?>>
						<a href="<?php  echo url('extension/pc/installed', array('type' => 'all'));?>">全部</a>
					</li>
					<?php  if(is_array($temtypes)) { foreach($temtypes as $type) { ?>
						<li <?php  if($_GPC['type'] == $type['name']) { ?> class="active" <?php  } ?>>
							<a href="<?php  echo url('extension/pc/installed', array('type' => $type['name']));?>"><?php  echo $type['title'];?></a>
						</li>
					<?php  } } ?>
				</ul>
			</div>
		...
		<div id="batch-contain" style="clear:both;height:50px;line-height:50px;margin-top:-20px;display:none">
			<a href="<?php  echo url('extension/pc/upgrade', array('check' => 1, 'batch' => 1));?>" class="btn btn-primary">一键更新模板</a>
		</div>
		<?php  if(is_array($templates)) { foreach($templates as $item) { ?>
			<div class="item">
				<div class="title">
					<div class="theme-title"><?php  echo $item['title'];?> (<?php  echo $item['name'];?>)</div>
					<img src="../app/pc/<?php  echo $item['name'];?>/preview.jpg" class="img-rounded" />
				...
				<div class="btn-group  btn-group-justified">
					<a title="卸载" onclick="return confirm('确定卸载此模板吗？')" href="<?php  echo url('extension/pc/uninstall', array('id' => $item['id']))?>" class="btn btn-default btn-xs">卸载</a>
				</div>
			</div>
		<?php  } } ?>
		<script type="text/javascript">
			var upgrade = [];
			$.post('<?php  echo url('extension/pc/check');?>', {foo: 'upgrade'},function(dat){
				try {
					...
					$('.upgrade-label').each(function(){
						...
						if(ret[n]) {
							...
							if(ret[n].version > v) {
								upgrade.push(n);
								$(this).html('<div class="img-rounded update-bg"></div><div class="update-div"><a href="<?php  echo url('extension/pc/upgrade')?>&templateid=' + n + '" class="btn-danger img-circle update-btn" title="来自系统云服务更新">更新</a></div>');
							}
						}
					});
					if(upgrade.length > 0) {
	...
	<?php  } ?>
	<?php  if($do == 'prepared') { ?>
		<h5 class="page-header">已购买的PC站模板</h5>
		<div class="form clearfix" ng-controller="listInstallThemes">
			<div class="item" id="templates-{{m.name}}" ng-repeat="m in templates">
				...
				<div class="btn-group  btn-group-justified">
					<a  href="<?php  echo url('extension/pc/install')?>templateid={{m.name.toLowerCase()}}" class="btn btn-default btn-xs">安装</a>
				</div>
			</div>
		</div>
		<?php  if($uninstallTemplates) { ?>
			<div class="form">
				<h5 class="page-header">未安装的PC站模板(本地模板)</h5>
			</div>
			...
			<?php  if(is_array($uninstallTemplates)) { foreach($uninstallTemplates as $item) { ?>
				<div class="item" id="templates-<?php  echo $item['name'];?>">
					...
					<div class="btn-group  btn-group-justified">
						<a  href="<?php  echo url('extension/pc/install', array('templateid' => $item['name']))?>" class="btn btn-default btn-xs">安装</a>
					</div>
				</div>
			<?php  } } ?>
		...
		<script type="text/javascript">
			require(['angular'], function(angular){
				angular.module('app', []).controller('listInstallThemes', function($scope, $http) {
					$.post('<?php  echo url('extension/pc/check');?>', {foo: 'install'},function(dat){
						try {
							var ret = $.parseJSON(dat);
							if(!$.isArray(ret)) {
								return;
				...
				var insta = function(){
					...
					if(!m_name) {
						util.message('本次成功安装' + success.length + '个模板.<br>安装失败' + fail.length + '个模板', "<?php  echo url('extension/pc/installed')?>", 'info');
						return;
					}
					...
					$.post("<?php  echo url('extension/pc/batch-install')?>", {'templateid' : m_name}, function(data){
						if(data == 'success') {
							i++;
