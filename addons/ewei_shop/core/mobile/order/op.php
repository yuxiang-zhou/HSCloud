...
if ($_W['isajax']) {
	if ($operation == 'cancel') {
		...
		if ($order['status'] != 0) {
			show_json(0, '粉单已支付，不能取消!');
		}
		pdo_update('ewei_shop_order', array('status' => -1, 'canceltime' => time()), array('id' => $order['id']));
		m('notice')->sendOrderMessage($orderid);
		if ($order['deductprice'] > 0) {
			$shop = m('common')->getSysset('shop');
			m('member')->setCredit($order['openid'], 'credit1', $order['deductcredit'], array('0', $shop['name'] . "购物返还抵扣积分 积分: {$order['deductcredit']} 抵扣金额: {$order['deductprice']} 粉单号: {$order['ordersn']}"));
		}
		...
		//增加的新内容----------
		$goodsid = pdo_fetchcolumn('select goodsid from ' . tablename('ewei_shop_order_goods') . ' where uniacid=:uniacid and orderid=:orderid limit 1',  array(':uniacid' => $uniacid, ':orderid' => $orderid));
		$isdiscount = pdo_fetchcolumn('select isdiscount from ' . tablename('ewei_shop_goods') . ' where uniacid=:uniacid and id=:goodsid limit 1',  array(':uniacid' => $uniacid, ':goodsid' => $goodsid));
		if ($isdiscount == 1) {
			show_json(0, '很抱歉，促销粉单是不允许退款的!');
		}
		//-----------------------
		if ($order['status'] != 1 && $order['status'] != 3) {
			show_json(0, '粉单未付款或未收货，不能申请退款!');
		} else {
			if ($order['status'] == 3) {
				if (!empty($order['virtual']) || $order['isverify'] == 1) {
					show_json(0, '此粉单不允许退款!');
				} else {
					...
					if ($refunddays > 0) {
						...
						if ($days > $refunddays) {
							show_json(0, '粉单完成已超过 ' . $refunddays . ' 天, 无法发起退款申请!');
						}
					} else {
						show_json(0, '粉单完成, 无法申请退款!');
					}
				}
			}
		}
