...
if ($operation == 'display') {
	...
	if (checksubmit('submit')) {
		...
		//修改加入进货返点
		$data = array('uniacid' => $_W['uniacid'], 'level' => intval($_GPC['level']), 'levelname' => trim($_GPC['levelname']), 'rate1' => floatval($_GPC['rate1']), 'rate2' => floatval($_GPC['rate2']), 'rate3' => floatval($_GPC['rate3']), 'ordercount' => intval($_GPC['ordercount']), 'ordermoney' => $_GPC['ordermoney'], 'discount' => $_GPC['discount']);
		if (!empty($id)) {
			if ($id == 'default') {
