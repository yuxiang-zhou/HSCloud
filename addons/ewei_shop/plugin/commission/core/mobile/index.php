...
if ($_W['isajax']) {
	...
	$today = getdate()[mday];
	$cansettle = $today >=10 && $today <=15 && $member['commission_ok'] >= 1 && $member['commission_ok'] >= floatval($this->set['withdraw']);
	...
	show_json(1, array('today' => $today, 'commission_ok' => $commission_ok, 'member' => $member, 'level' => $level, 'cansettle' => $cansettle, 'settlemoney' => number_format(floatval($this->set['withdraw']), 2), 'set' => $this->set,));
}
